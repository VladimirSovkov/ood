...
TEST_CASE("GumballMachine with a naive config is in sold out state")
{
...
TEST_CASE("Gumball Machine with a naive config is in no qarter state")
{
...
TEST_CASE("GumballMachine with a naive config is in has quarter state")
{
	std::ostringstream ss;
	RedirectCout buff(ss.rdbuf());
...
TEST_CASE("Gumball Machine with a naive config is in max quarter count state")
{
	CGumballMachine machine(5);
	machine.InsertQuarter();
	machine.InsertQuarter();
	...
	machine.InsertQuarter();
	machine.InsertQuarter();
	CHECK(CreateStateGumballMachine(5, 5, "waiting for turn of crank with maximum count quarters") == machine.ToString());
