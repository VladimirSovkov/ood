...
{
public: 
	CChocolate(IBeveragePtr&& beverage, size_t quantity)
		:CCondimentDecorator(move(beverage))
	{
		m_quantity = std::min(quantity, MAX_NUMBER_COUNT);
	}

	const size_t MAX_NUMBER_COUNT = 5;
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return std::to_string(m_quantity) + " slices of chocolate " ;
	}
